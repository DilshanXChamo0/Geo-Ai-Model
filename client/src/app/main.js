...
function Main() {
    ...
    function fetchConversation(id) {
        ...
        axios.get(`https://geo-server.duckdns.org/api/v1/get-conversation/${id}`)
            .then(res => {
    ...
    const handleSend = async (e) => {
        ...
        await axios.post('https://geo-server.duckdns.org/api/v1/save-conversation', {
            conversationId: id,
            chatId,
        ...
        try {
            const response = await axios.post('https://geo-server.duckdns.org/api/v1/generate-image', {
                prompt,
            ...
            if (response.data.success) {
                ...
                await axios.post('https://geo-server.duckdns.org/api/v1/save-conversation-image', {
                    conversationId: id,
                    chatId,
                    images: images,
                }).catch(error => console.warn(error));
